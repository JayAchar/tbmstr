render_docx <- function(tables, output_dir) {
  ...
  tables_doc <- officer::read_docx() %>%
    ...
    officer::body_add_par("Treatment cohort description", style = "heading 1") %>%
    officer::body_add_par("Main Table 1", style = "heading 2") %>%
    body_add_flextable(value = tables$mt1) %>%
    body_add_break() %>%
    body_add_par("Treatment cohort outcomes", style = "heading 1") %>%
    body_add_par("Main Table 2", style = "heading 2") %>%
    ...
    body_add_par("Main Table 3", style = "heading 2") %>%
