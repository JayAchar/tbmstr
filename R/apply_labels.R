...
apply_labels <- function(df, variable) {
  ...
  if(! variable %in% unique(lut$name)) {
    cli::cli_warn("`apply_labels()`: `{variable}` is not found in the variable look up table")
    return(df[[variable]])
  }
  
  filtered_lut <- lut[which(lut$name == variable), ]
  
  assigned <- filtered_lut$description[match(df[[variable]], 
                                 filtered_lut$value)]
  ...
  if(! original_na == assigned_na) {
    cli::cli_warn(
      "`apply_labels()`: {variable}: {assigned_na - original_na} missing value{?s} have been introduced"
    )
  }
